...
const createOrder = async (req, res) => {
  try {
    ...
    // DEBUG LOGS
    console.log('==============================');
    console.log('Restaurant ordered:', restaurant?.name);
    ...
    for (const item of items) {
      ...
      if (!menuItem) {
        return res.status(400).json({
          success: false,
          message: `Item ${item.menuItemId} not found`,
        });
      }
    ...
    console.log('Saved Order Restaurant:', order.restaurant);
  ...
  try {
    ...
    const [orders, total] = await Promise.all([
      Order.find(query)
        .populate(
          'restaurant',
          'name image address cuisineType'
        )
        .sort({ createdAt: -1 })
        .skip(skip)
...
const getOrder = async (req, res) => {
  try {
    const order = await Order.findById(req.params.id)
      .populate(
        'restaurant',
        'name image address phone location'
      )
      .populate('user', 'name email phone')
...
const updateOrderStatus = async (req, res) => {
  try {
    const { status, note, courierLocation } =
      req.body;
...
const getRestaurantAnalytics = async (
  req,
  res
) => {
  try {
